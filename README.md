...
## BPM 取整數(錄音室假設)
- 偵測到的 BPM 會精修到**整數**:在偵測值的 1×/2×/½× 三個窄帶用 `lockScore`(onset 對該速度的鎖相度)找峰,取最高再四捨五入。
- 為什麼:錄音室幾乎都對著整數 click 錄音;用精確整數可**消除「BPM 估計誤差造成的假漂移」**(實測「大山」從 81ms 假漂移降到 10ms)。
- 會選到「鼓點最密的主脈動」(此曲=152,正是團員腳踩的),要慢一半按 ÷2。非整數歌(現場)才保留一位小數。
